...
const CodeEditor = () => {
  ...
  useEffect(() => {
    setIsLoaded(true);
    // Delay the main content appearance
      ...
      <Box position="relative" zIndex={1}>
        ...
        <Fade in={showMainContent}>
          <MotionHStack 
            ...
            mx={10} 
            my={1}
            // --- CHANGE --- Make children stretch to equal height
            align="stretch"
          ...
          >
            {/* Left Column: Editor */}
            <MotionBox 
              // --- CHANGE --- Use flex={1} for an equal 50% split with the other column
              flex={1}
            ...
            <MotionBox
              flex={1}
              // --- CHANGE --- Make this box a flex container so its child (Output) can fill its space
              display="flex"
              ...
              whileHover={{ scale: 1.002 }}
            >
              {/* --- CHANGE --- Ensure the Fade component takes up all available flex space */}
              <ScaleFade 
                in={isLoaded} 
                initialScale={0.95} 
