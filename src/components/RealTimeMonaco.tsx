...
> = ({ ...props }) => {
  ...
  const { name, color, collaborateId } = {
    name: props.name,
    color: props.color,
    collaborateId: props.roomId,
  };
  useEffect(() => {
          ...
          if (item.indexOf(backgroundC) !== -1) {
            ...
            if (myState?.color !== `#${color}`) {
              if (decorate && !elements[i].textContent) {
                const ind = allUsers.findIndex(
                  (elem: any) => elem.user?.color === `#${color}`
                );
                (elements[i] as HTMLElement).innerHTML = "";
                decorateElement(
                  elements[i] as HTMLElement,
                  color,
                  allUsers[ind]?.user?.name ?? `#${color}`
                );
              } else {
    ...
    const applyColorElements =
      document.getElementsByClassName("applyColorToThis");
    ...
    if (awarenessElem) {
      awarenessElem.on("update", () => {
        const states = Array.from(
          awarenessElem.getStates().values()
        ) as UsersType[];
  ...
  function decorateCursors(
    newSelections: SelectionsType[],
    newCursors: CursorsType[],
    users: string[],
    ...
    for (let p = 0; p < newCursors.length; p++) {
      ...
      r.push({
        ...
        options: {
          className: ` applyColorToThis ${
            myState?.color === users[p] ? "" : "my-cursor"
          } backgroundC-${users[p]?.slice(1)}`,
        },
      });
    }
    for (let p = 0; p < newSelections?.length; p++) {
      ...
      if (curr !== null) {
        r.push({
          ...
          options: {
            className: `${
              myState?.color === users[p] ? "" : "applyColorToThis"
            } backgroundC-${users[p]?.slice(1)}`,
          },
        });
      }
